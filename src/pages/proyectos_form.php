...
if (isset($_GET['id'])) {
    ...
    try {
        $pdo = getDbConnection();
        $stmt = $pdo->prepare("CALL sp_read_proyecto_by_id(?)");
...
<header>
    <h1><?= $is_edit ? 'Editar' : 'Añadir' ?> Proyecto</h1>
</header>
<section class="form-container">
    <form action="../src/actions/proyectos_process.php" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="date" id="fecha_inicio" name="fecha_inicio" value="<?= htmlspecialchars($item['fecha_inicio'] ?? '') ?>">
        </div>
        <div class="form-group">
            <label for="fecha_fin">Fecha Fin</label>
            <input type="date" id="fecha_fin" name="fecha_fin" value="<?= htmlspecialchars($item['fecha_fin'] ?? '') ?>">
        </div>
        <div class="form-group">
        ...
        <button type="submit" class="btn-submit"><?= $is_edit ? 'Actualizar' : 'Crear' ?> Proyecto</button>
    </form>
</section>

...
try {
    $pdo = getDbConnection();
    $stmt = $pdo->prepare("CALL sp_read_all_tipos_auxiliar(?, ?)");
    $stmt->execute([$filter_codigo, $filter_nombre]);
    $items = $stmt->fetchAll();
} catch (PDOException $e) {
    die("Error al obtener los tipos de auxiliar: " . $e->getMessage());
}
?>
...
<header>
    <h1>Mantenimiento de Tipos de Auxiliar</h1>
</header>
<section>
    <a href="index.php?page=tipos_auxiliar_form" class="btn btn-add">Añadir Nuevo Tipo de Auxiliar</a>

    <form action="index.php" method="GET" class="filter-form">
        <input type="hidden" name="page" value="tipos_auxiliar">
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="index.php?page=tipos_auxiliar_form&id=<?= $item['id'] ?>" class="btn btn-edit">Editar</a>
                    <a href="../src/actions/tipos_auxiliar_process.php?action=delete&id=<?= $item['id'] ?>" class="btn btn-delete" onclick="return confirm('¿Está seguro?');">Eliminar</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>

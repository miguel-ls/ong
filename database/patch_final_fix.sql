...
-- Fecha: 2025-09-07
-- Autor: Jules AI
-- Descripción: Este script está diseñado para ser ejecutado en cualquier
-- estado de la base de datos (ya sea que los parches anteriores
-- fallaron, se ejecutaron parcialmente o no se ejecutaron).
-- Dejará la base de datos en el estado final correcto para la
-- funcionalidad de 'Tipos de Documento de Identidad'.
-- =================================================================

-- == PARTE 1: TABLAS Y DATOS INICIALES ===============================

-- Crear la tabla de tipos de documento de identidad si no existe.
...
-- Poblar la tabla con datos iniciales. INSERT IGNORE no insertará si el código ya existe.
...
-- == PARTE 2: CORRECCIÓN DE LA TABLA `auxiliares` =====================

-- Procedimiento seguro para alterar la tabla `auxiliares`.
DROP PROCEDURE IF EXISTS `patch_fix_auxiliares_table`;
DELIMITER $$
CREATE PROCEDURE `patch_fix_auxiliares_table`()
BEGIN
    -- 1. Añadir la nueva columna si no existe.
    ...
    -- 2. Migrar datos si la columna vieja existe.
    ...
    -- 4. Añadir la clave foránea si no existe.
...
-- Ejecutar el procedimiento de corrección y luego eliminarlo.
CALL `patch_fix_auxiliares_table`();
DROP PROCEDURE `patch_fix_auxiliares_table`;


-- == PARTE 3: RE-CREACIÓN DE TODOS LOS PROCEDIMIENTOS ALMACENADOS RELACIONADOS =====
...
-- SPs para `auxiliares` (versiones finales y correctas)
